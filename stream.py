...
from .config import (
    ARM_LINES,
    CLAW_MAX_ANGLE,
    CLAW_PROXIMITY_THRESHOLD_PX,
    POSE_THRESHOLD,
    SERVO_MAX_ANGLE,
    SERVO_MIN_ANGLE,
)
from .pose_math import arm_is_flexed, angle_degrees, get_keypoints, hand_distance
...
def generate_frames(*, engine, hw, input_width: int, input_height: int):
    ...
    while True:
        ...
        for pose in poses:
            ...
            dist = hand_distance(points)
            if dist is not None:
                claw_dist = dist
                if dist >= CLAW_PROXIMITY_THRESHOLD_PX:
                    claw_angle_val = SERVO_MIN_ANGLE
                else:
                    t = 1.0 - (dist / CLAW_PROXIMITY_THRESHOLD_PX)
                    claw_angle_val = t * CLAW_MAX_ANGLE
                hw.set_claw(claw_angle_val)
        ...
        if claw_dist is not None:
            state = "CLOSED" if claw_angle_val >= CLAW_MAX_ANGLE * 0.95 else ("CLOSING" if claw_angle_val > 0 else "OPEN")
            cv2.putText(
                frame,
                f"Claw: {claw_angle_val:.1f}deg  dist={claw_dist:.0f}px  [{state}]",
                (10, status_y),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.6,
                (0, 180, 255),
